...
<div class="container aboutus margintop60" >
    ...
    <div class="row subaboutus">
        ...
        <div class="col-md-3">
            <div class="list-group">
                ...
                <a href="complete.php" class="list-group-item active">COMPLETED</a>
            </div>
        </div>
                ...
                <div data-u="slides" style="cursor: default; position: relative; top: 0px; left: 240px; width: 720px; height: 480px; overflow: hidden;">
                    <?php while ($upcoming_info = mysqli_fetch_assoc($quiry_result)) { ?>
                        <div data-p="150.00" style="display: none;">
                            ...
                            <a href="singleproject.php?id=<?= $upcoming_info['id']; ?>&category=tbl_completed_project"> <img data-u="image" src="images/<?= $upcoming_info['image']; ?>" />  </a>
    ...
    <div class="row subaboutus text-center">

        <form class="form-inline" action="complete.php" method="post">
            ...
            <select name="city" class="form-control" >
             ...
             //  $conn= mysqli_connect("127.0.0.1", "ambitbd_root", "S]67U+c};-bR@mWB", "ambitbd_realstate");
                $sql = "SELECT DISTINCT  city FROM tbl_completed_project";
            ...
            <select name="location" class="form-control">
                <?php
                $sql = "SELECT DISTINCT  project_location FROM tbl_completed_project";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ?>
    ...
    if (isset($_POST['Search'])) {
        ...
        $sql = "SELECT * FROM tbl_completed_project where city='$city' and project_location='$location'";
        // echo $sql;
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            ?>
            <div class="row">
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    <div class="col-md-4">
                        <br>
                        <a href="singleproject.php?id=<?= $row['id']; ?>&category=tbl_completed_project"> 

                            <img src="images/<?= $row['image']; ?>" style="width:250px; height: 250px">
                            <h4 class="text-danger"> <?= $row['project_name']; ?></h4>
                        </a>
                    </div>
    ...
    <?php
    $quiry_result = $obj_application->select_all_completed_project_info();
    while ($upcoming_info = mysqli_fetch_assoc($quiry_result)) {
        ?>
        <div class="col-md-3">
            <h4><?php echo $upcoming_info['project_name']; ?></h4><br>
            <a href="singleproject.php?id=<?= $upcoming_info['id']; ?>&category=tbl_completed_project"> <img class="img img-responsive img-thumbnail" src="images/<?php echo $upcoming_info['image']; ?>"></a>
        </div> 
    <?php } ?>
